...
class FurnitureApp:
    ...
    def load_cabinets(self, filename):
        ...
        grouped = df.groupby('Nazwa').apply(lambda x: x[
            x.columns.intersection(['partName', 'height', 'width', 'pieces', 'wrapping', 'comments'])].to_dict(
            'records'))
    ...
    def create_widgets(self):
        ...
        column_1_low, column_2_low, column_3_low = split(list(self.lower_cabinets.keys()), 3)
        column_1_upp, column_2_upp, column_3_upp = split(list(self.upper_cabinets.keys()), 3)
        ...
        def create_buttons(column, column_frame):
            for cabinet in column:
                ...
                cabinet_label = tk.Label(cabinet_frame, text=cabinet, font=("Arial", 12))
                cabinet_label.pack(side=tk.LEFT, padx=5)
                plus_button = tk.Button(cabinet_frame, text="+", font=("Arial", 12), width=2,
                                        command=lambda selected=cabinet: self.add_to_cart(selected))
                plus_button.pack(side=tk.RIGHT, padx=5)
                minus_button = tk.Button(cabinet_frame, text="-", font=("Arial", 12), width=2,
        ...
        print_button = tk.Button(cart_frame, text="Zapisz", font=("Arial", 12),
                                 command=lambda cabinets=self.cabinets, entry_button=cart_entry,
                                                color_entry=color_entry: self.save_cart(
    ...
    def save_cart(self, cabinets, entry_button, color_entry):
        ...
        for part in parts_list:
            ...
            for existing_part in parts_list_sorted:
                if part['partName'] == existing_part['partName'] and part['height'] == existing_part['height'] and part[
                    'width'] == existing_part['width'] and part['comments'] == existing_part['comments'] and part[
                    'wrapping'] == existing_part['wrapping']:
